...
nNode* createNode(char name[])
{
    ...
    strcpy( node->filedescriptor.file_name, name );
    node->child = NULL;
    node->sibling = NULL;
...
nNode * searchForNodeInAllSiblings( nNode * t , char name[] )
{
    ...
    while( t != NULL )
    {
        if( strcmp( t->filedescriptor.file_name, name) == 0 )
            return t;
        t = t->sibling;
    }
    return NULL;
...
nNode * insertNode_fd(nNode *root, file_descriptor filedescriptor)
{
    nNode *ins;
    char nName[MAX_LEVELS][MAX_LENGTH];
    int count = splitPath( filedescriptor.location_full_path, nName);
    int i = 0;
    ...
    if( count == 0 && root == NULL )
    {
        ins = createNode(filedescriptor.file_name); // expecting root node name is "/"
        return ins;
    }
    ...
    if( count == 0 && root != NULL )
    {
        ...
        root->child = insertAtEnd( root->child , filedescriptor.file_name  );
    ...
    matchedNode->child = insertAtEnd( matchedNode->child , filedescriptor.file_name  );
...
nNode * removeLinkFromList( nNode * list , char * nodeName )
{
    ...
    while( t != NULL )
    {
        ...
        if( strcmp( t->filedescriptor.file_name, nodeName )  == 0 )   // found
        {
